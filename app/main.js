...
app.get('/landing', (req, res) => {
  ...
  if (d)
    res.redirect(`/device/${d.deviceId}`) // Redirect to device display // If not recognized, display information
  else
...
app.get('/device/new', (req, res) => {
  Channel.find()
    ...
    .exec((err, channels) => {
      ...
      res.render('index', {
        render: 'device',
        devices: devices.filter(d => d.unregistered),
        channels: channels
      })
    })
})
...
app.post('/device/new', (req, res) => {
  ...
  c.save((err, device) => {
    ...
    var c = clients.find(
      c => stripIPv6(c.handshake.address) == devices[i].address
    )
    if (c)
      c.emit('register', devices[i].deviceId) // emit 'register' to have setup page redirected
...
app.get('/device/:device_id', (req, res) => {
  Chromecast.findOne({ deviceId: req.params.device_id })
    .populate('channel')
    .exec((err, device) => {
      if (err) console.log(err)
      if (device && device.channel) {
        if (takeover)
          res.render(`layouts/${takeover.layout}`, {
          ...
          display device page */
          res.render(`layouts/${device.channel.layout}`, {
            deviceId: req.params.device_id,
...
app.get('/device/:device_id/connect', (req, res) => {
  ...
  if (d.status == 'offline' || d.status == 'waiting') launchHub(d.address)
  else {
    ...
    .exec((err, device) => {
      ...
      Channel.find()
        ...
        .exec((err, channels) => {
          if (err) console.log(err)
          if (device) {
            var localDevice = devices.find(
              d => d.deviceId == req.params.device_id
            )
            res.render('index', {
              render: 'device',
              device: Object.assign(localDevice, device),
...
app.post('/channel/:channel_id/edit', (req, res) => {
  ...
  for (var i in devices) {
    ...
    if (
      d.channel &&
      d.channel._id.toString() == req.params.channel_id.toString()
    )
...
app.delete('/channel/:channel_id/edit', (req, res) => {
  Channel.remove({ _id: req.params.channel_id }, () => {
    ...
    Chromecast.update(
      { channel: new mongoose.Types.ObjectId(req.params.channel_id) },
      { $unset: { channel: 1 } },
      (err, numAffected, result) => {
        res.sendStatus(200)
      }
    )
  })
...
app.post('/new/takeover', (req, res) => {
  Channel.findOne({
    _id: new mongoose.Types.ObjectId(req.body.channel_id)
  }).exec((err, channel) => {
    if (err) console.log(err)
    takeover = channel
    clients.forEach(c => {
